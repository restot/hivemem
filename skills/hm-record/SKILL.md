...
2. **Always capture git commit**: Run `git rev-parse --short HEAD` in the current directory. This SHA will be attached as evidence for staleness tracking.
3. If no arguments: scan the session for learnings. For each one, generate title + content + summary + type + tags and record via the CLI. Report what was recorded.
4. If arguments provided:
   ...
   c. **Generate a summary**: 1-2 sentences that capture the key point. This appears in search results so make it useful for scanning.
